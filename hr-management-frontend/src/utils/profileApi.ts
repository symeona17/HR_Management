...
export async function fetchTrainerFeedback(trainerId: number) {
  const res = await apiFetch(`${API_BASE_URL}/trainer/${trainerId}/feedback`);
  return res.json();
}
...
export async function fetchManagerAnalytics(managerId: number) {
  const res = await apiFetch(`${API_BASE_URL}/manager/${managerId}/team-analytics`);
  return res.json();
}

export async function managerAssignTraining(employeeId: number, trainingId: number) {
  const res = await apiFetch(`${API_BASE_URL}/manager/assign-training`, {
    method: 'POST',
